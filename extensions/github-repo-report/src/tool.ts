...
export function createGithubRepoReportTool(
  ...
  return {
    ...
    parameters: createGithubRepoReportSchema(listGithubRepoSlugs(registry)),
    execute: async (_toolCallId, params) => {
      const args = params as { repo: string; ref?: string };
      const entry = resolveGithubRepoEntry(registry, args.repo);
      if (!entry) {
      ...
      if (!isGithubRepoEnabled(registry, args.repo)) {
        throw new Error(
          `Repo "${args.repo}" is not enabled yet. Edit the plugin config to enable it.`,
        );
      }
